        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div className="card p-3 sm:p-4">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <div className="flex-shrink-0">
                  <svg className="h-4 w-4 sm:h-5 sm:w-5 text-secondary" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                  </svg>
                </div>
                <div className="flex-1">
                  <h3 className="text-xs sm:text-sm font-medium text-secondary mb-2">Errori di validazione:</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
            <div>
              ...
              <input
                ...
                className="lux-input w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base"
            ...
            <div>
              ...
              <select
                ...
                className="lux-select w-full px-3 sm:px-4 py-2 sm:py-3 rounded-xl text-sm sm:text-base"
              >
            ...
            <div>
              ...
              <select
                ...
                className="lux-select w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base rounded-xl"
              >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="lux-textarea w-full px-3 sm:px-4 py-2 text-sm sm:text-base"
          ...
          <div>
            ...
            <div className="space-y-4">
              {bets.map((bet, index) => (
                <div key={bet.id} className="card p-3 sm:p-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="lux-select w-full px-3 py-2 text-xs sm:text-sm"
                      >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
            <div>
              ...
              <div className="relative">
                <DollarSign className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 h-3 w-3 sm:h-4 sm:w-4 text-secondary" />
                <input
                  type="number"
                  required
                  min="0.01"
            ...
            <div>
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <span className="text-xs sm:text-sm text-secondary w-6 sm:w-8">{formData.confidence_level}</span>
              </div>
            </div>
          </div>
          ...
          {/* Riepilogo */}
          <div className="card p-3 sm:p-4">
            <h4 className="text-base sm:text-lg font-semibold text-primary mb-2 sm:mb-3">Riepilogo</h4>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 text-xs sm:text-sm">
              <div>
                <span className="text-secondary">Quota Totale:</span>
                <p className="text-primary font-semibold">{calculateTotalOdds().toFixed(2)}</p>
              </div>
              <div>
                <span className="text-secondary">Stake:</span>
                <p className="text-primary font-semibold">{formData.stake.toFixed(2)} {currency}</p>
              </div>
              <div>
                <span className="text-secondary">Vincita Potenziale:</span>
                <p className="text-accent-gold font-semibold">{calculatePotentialWin().toFixed(2)} {currency}</p>
              </div>
              <div>
                <span className="text-secondary">Profitto Potenziale:</span>
                <p className="text-accent-gold font-semibold">{(calculatePotentialWin() - formData.stake).toFixed(2)} {currency}</p>
              </div>
            </div>
          </div>

          {/* Buttons */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end space-y-2 sm:space-y-0 sm:space-x-4 pt-3 sm:pt-4 border-t border-white/10">
            <button
              type="button"
              onClick={onClose}
              className="btn-secondary px-4 sm:px-6 py-2 text-sm sm:text-base"
            >
              Annulla
            </button>
            <button
              ...
              className="btn-primary flex items-center justify-center space-x-2 px-4 sm:px-6 py-2 text-sm sm:text-base disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
                <>
