...
const GroupsManager = ({ currency }: GroupsManagerProps) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'won': return 'text-accent-gold'
      case 'lost': return 'text-secondary'
      case 'void': return 'text-secondary'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-primary">Gruppi di Pronostici</h2>
          <p className="text-secondary mt-1 text-sm sm:text-base">Organizza i tuoi pronostici in gruppi per analisi dettagliate</p>
        </div>
        <button
          onClick={() => setShowCreateForm(true)}
          className="btn-primary flex items-center justify-center space-x-2 px-4 sm:px-6 py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all duration-200 min-h-[44px] w-full sm:w-auto"
        >
      ...
      {groups.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {groups.map(group => (
            <div key={group.id} className="card p-6 hover:border-white/30 transition-all duration-300">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                  ...
                  />
                  <h3 className="text-base sm:text-lg font-semibold text-primary truncate">{group.name}</h3>
                </div>
                <div className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
                  <button
                    onClick={() => handleShowGroupDetails(group)}
                    className="btn-secondary p-2 min-h-[44px] min-w-[44px] flex items-center justify-center"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-xs sm:text-sm">
                <div className="col-span-1">
                  <span className="text-secondary block">Pronostici:</span>
                  <p className="text-primary font-semibold">{group.predictions_count}</p>
                </div>
                <div className="col-span-1">
                  <span className="text-secondary block">Win Rate:</span>
                  <p className="text-primary font-semibold">{group.win_rate.toFixed(1)}%</p>
                </div>
                <div className="col-span-2 sm:col-span-1">
                  <span className="text-secondary block">Stake Totale:</span>
                  <p className="text-primary font-semibold text-xs sm:text-sm">{group.total_stake.toFixed(2)} {currency}</p>
                </div>
                <div className="col-span-1">
                  <span className="text-secondary block">Profitto:</span>
                  <p className={`font-semibold text-xs sm:text-sm ${
                    group.total_profit >= 0 ? 'text-accent-gold' : 'text-secondary'
                  }`}>
                    {group.total_profit >= 0 ? '+' : ''}{group.total_profit.toFixed(2)} {currency}
                  </p>
                </div>
                <div className="col-span-1">
                  <span className="text-secondary block">ROI:</span>
                  <p className={`font-semibold ${
                    group.roi >= 0 ? 'text-accent-gold' : 'text-secondary'
                  }`}>
                    {group.roi >= 0 ? '+' : ''}{group.roi.toFixed(1)}%
                  </p>
                </div>
                <div className="col-span-1">
                  <span className="text-secondary block">Quota Media:</span>
                  <p className="text-primary font-semibold">{group.avg_odds.toFixed(2)}</p>
                </div>
              </div>

              <div className="mt-4 pt-4 border-t border-white/10">
                <span className="text-xs text-secondary">
                  Creato il <time dateTime={group.created_at} suppressHydrationWarning>{new Date(group.created_at).toLocaleDateString('it-IT', { timeZone: 'UTC' })}</time>
                </span>
              </div>
            </div>
          ))}
            ...
            <form onSubmit={handleCreateGroup} className="p-4 sm:p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="lux-input w-full px-4 py-2"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="lux-textarea w-full px-4 py-2"
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
                <div className="card p-4">
                  <div className="text-2xl font-bold text-primary">{selectedGroup.predictions_count}</div>
                  <div className="text-sm text-secondary">Pronostici</div>
                </div>
                <div className="card p-4">
                  <div className="text-2xl font-bold text-primary">{selectedGroup.win_rate.toFixed(1)}%</div>
                  <div className="text-sm text-secondary">Win Rate</div>
                </div>
                <div className="card p-4">
                  <div className={`text-2xl font-bold ${
                    selectedGroup.total_profit >= 0 ? 'text-accent-gold' : 'text-secondary'
                  }`}>
                    {selectedGroup.total_profit >= 0 ? '+' : ''}{selectedGroup.total_profit.toFixed(2)} {currency}
                  </div>
                  <div className="text-sm text-secondary">Profitto</div>
                </div>
                <div className="card p-4">
                  <div className={`text-2xl font-bold ${
                    selectedGroup.roi >= 0 ? 'text-accent-gold' : 'text-secondary'
                  }`}>
              ...
              <div>
                <h4 className="text-lg font-semibold text-primary mb-4">Pronostici del Gruppo</h4>
                {groupPredictions.length === 0 ? (
                  <div className="text-center text-secondary py-8">
                    <Target className="h-12 w-12 mx-auto mb-3 text-accent-gold opacity-50" />
                    ...
                    {groupPredictions.map(prediction => (
                      <div key={prediction.id} className="card p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h5 className="text-primary font-medium">{prediction.title}</h5>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 text-xs sm:text-sm">
                          <div>
                            <span className="text-secondary block">Sport:</span>
                            <p className="text-primary">{prediction.sport}</p>
                          </div>
                          <div>
                            <span className="text-secondary block">Quota:</span>
                            <p className="text-primary">{prediction.total_odds.toFixed(2)}</p>
                          </div>
                          <div>
                            <span className="text-secondary block">Stake:</span>
                            <p className="text-primary">{prediction.stake.toFixed(2)} {currency}</p>
                          </div>
                          <div>
                            <span className="text-secondary block">Data:</span>
                            <p className="text-primary"><time dateTime={prediction.event_date} suppressHydrationWarning>{new Date(prediction.event_date).toLocaleDateString('it-IT', { timeZone: 'UTC' })}</time></p>
                          </div>
                        </div>
                        {prediction.result_profit !== undefined && (
                          <div className="mt-2 pt-2 border-t border-white/10">
                            <span className={`text-sm font-medium ${
                              prediction.result_profit > 0 ? 'text-accent-gold' : 
                              prediction.result_profit < 0 ? 'text-secondary' : 'text-secondary'
                            }`}>
